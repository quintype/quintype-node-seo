...
function pickImageFromCard(story, cardId) {
  ...
  if (
    ...
    return new FocusedImage(
      metadata["social-share"].image.key,
      metadata["social-share"].image.metadata || {}
    );
  }
}
...
function pickImage(pageType, data, url) {
  if (pageType === "story-page" && url.query && url.query.cardId) {
    ...
    return (
      pickImageFromCard(story, url.query.cardId) || pickImageFromStory(story)
    );
