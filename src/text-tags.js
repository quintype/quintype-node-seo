...
function buildTagsFromStory(config, story, url = {}) {
  ...
  function getStoryCardMetadata(cardId) {
    const { metadata = {} } =
      story.cards.find(card => card.id === cardId) || {};
    const urlWithCardId = `${config["sketches-host"]}/${
      story.slug
    }?cardId=${cardId}`;
  ...
  const storyMetaData = {
    ...
    keywords: (
      seo["meta-keywords"] || (story.tags || []).map(tag => tag.name)
    ).join(","),
...
function getSeoData(config, pageType, data, url = {}, seoConfig = {}) {
  function findRelevantConfig(pred) {
    ...
    if (
      seoMetadata.data &&
      (!seoMetadata.data["page-title"] || !seoMetadata.data["title"])
    ) {
      const sectionData =
        sections.find(section => section.id === seoMetadata["owner-id"]) || {};
      seoMetadata.data["page-title"] =
        seoMetadata.data["page-title"] || sectionData.name;
      seoMetadata.data["title"] = seoMetadata.data["title"] || sectionData.name;
    }
    if (seoMetadata.data && !seoMetadata.data["description"]) {
      const homeSeoData =
        config["seo-metadata"].find(page => page["owner-type"] === "home") ||
        {};
      seoMetadata.data["description"] =
        homeSeoData.data && homeSeoData.data.description
          ? homeSeoData.data.description
          : "";
    }
    if (seoMetadata.data) {
  ...
  switch (pageType) {
    case "home-page":
      return findRelevantConfig(page => page["owner-type"] === "home");
    case "section-page":
      return (
        findRelevantConfig(
          page =>
            page["owner-type"] === "section" &&
            page["owner-id"] === get(data, ["data", "section", "id"])
        ) || getSeoData(config, "home-page", data, url)
      );
    case "tag-page":
      return (
        buildTagsFromTopic(config, get(data, ["data", "tag"]), url) ||
        getSeoData(config, "home-page", data, url)
      );
    case "story-page":
      return (
        buildTagsFromStory(config, get(data, ["data", "story"]), url) ||
        getSeoData(config, "home-page", data, url)
      );
    default:
...
export function TextTags(seoConfig, config, pageType, data, { url }) {
  ...
  const twitterTags = seoConfig.enableTwitterCards
    ? {
        ...
        "twitter:title":
          pageType === "story-page"
            ? get(data, ["data", "story", "headline"], "error")
            : seoData.title,
  ...
  if (pageType === "story-page" && seoConfig.enableNews) {
    ...
    if (get(data, ["data", "story", "seo", "meta-google-news-standout"]))
      commonTags.push({
        tag: "link",
        rel: "standout",
        href: seoData.storyUrl || currentUrl
      });
  }
...
export function getTitle(seoConfig, config, pageType, data, params) {
  if (get(data, ["title"])) return get(data, ["title"]);

  if (get(data, ["data", "title"])) return get(data, ["data", "title"]);
