...
function getSeoData(config, pageType, data, url = {}, seoConfig = {}) {
  ...
  switch(pageType) {
    ...
    case 'amp-story-page': return buildTagsFromStory(config, get(data, ["data", "story"]), url) || getSeoData(config, "home-page", data, url);
...
export function TextTags(seoConfig, config, pageType, data, {url}) {
  ...
  const ogTags = seoConfig.enableOgTags ? {
    'og:type': pageType === 'story-page' || pageType === 'amp-story-page' ? 'article' : 'website',
  ...
  if((pageType === 'story-page' || pageType === 'amp-story-page') && seoConfig.enableNews) {
